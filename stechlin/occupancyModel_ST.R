...
cat("
    ...
    for (i in 1:M) {
        ...
        logit(psi[i]) <- beta0 + beta1 * Al[i] + beta2 * Pb[i] + beta3 * ddt[i] + random.beta.otu[OTU[i]]
...
params <- c("alpha0", "alpha1", "beta0", "beta1", "beta2","beta3") 

# MCMC settings
# ni <- 250000   ;   nt <- 100   ;   nb <- 50000   ;   nc <- 3
ni <- 250   ;   nt <- 10   ;   nb <- 50   ;   nc <- 1
